...
% [ ] Laurearsi
% [ ] Spazio di Bruillon?
% [ ] Aggiustare dimostrazione catena con q_m e N=2p+1
% [ ] Sposstare relazione di dispersione e prima parte eq moto nella sottosez precedente
...
\chapter{L'Oscillatore Armonico}\label{ch:1}
    ...
    \end{equation} che non \`e altro che una forma quadratica del tipo $\displaystyle\frac{1}{2}\underline{x}^\top H_V\qty(\vb{0})\underline{x}$, con $\underline{x}\in\R^n$ un punto e $H_V\qty(\vb{0})$ la matrice Hessiana di $V$ calcolata in $\vb{0}$. \par Ovviamente questo \`e soltanto un modello approssimato che pu\`o essere applicato in un intorno del minimo. Se ci si allontana troppo, le discordanze tra il modello e le misure che si riscontrano prendono il nome di \emph{effetti anarmonici}.

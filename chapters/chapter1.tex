...
\section{Oscillatore a $N$ Gradi di Libert\`a}
    ...
    \end{equation}
    %Per la \eqref{eq:QQ} possiamo scrivere $\rho_q^-e^{-i\qty(qx_n-\Omega_qt-\phi_q^-)}=\rho_{-q}^-e^{i\qty(qx_n-\Omega_{-q}t-\phi_{-q}^-)}$, dove tuttavia $\rho_{-q}^-=\rho_{q}^-$ in quanto il modulo di un numero complesso \`e uguale al modulo del suo coniugato, $\Omega_{-q}=-\Omega_q$ in quanto $\Omega_q$ dipende dal seno di $q$, che \`e una funzione dispari e infine $\phi_{-q}^-=-\phi_q^-$ l'argomento di un numero complesso cambia segno passando al coniugato. Di conseguenza: $$\zeta_n=\frac{1}{\sqrt{N}}\sum_q\qty[\rho_q^+e^{i\qty(qx_n+\Omega_qt+\phi_q^+)}+\rho_q^-e^{i\qty()}]$$
    
    %Come gi\`a detto in precedenza, la parte reale di queste soluzioni \`e costituita da una somma di seni e coseni, proprio come ci aspettavamo. Notiamo inoltre che anche questa volta per $q=0$ si ha il moto di traslazione che non influenza la dinamica del sistema.
